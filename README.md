...
[openclaw](https://github.com/openclaw/openclaw) 上で動作し、毎朝7時（JST）に自動でニュースが流れ、Telegramにレポートが届く。
...
[ファクトチェッカー] 概ね正確。「6兆円」は誇張。元記事は「最大400億ドル」  ✅
    ↓
[Observer → Telegram] 📊 日次レポート: 事実2件 / 要検証2件 / デマ拡散率 50%
```
...
| エージェント | 役割 | 実行スケジュール |
|------------|------|----------------|
| 📰 Journalist | 実ニュースをWeb検索して投稿 | 毎日 JST 7:00 |
| 📢 Spreader | ニュースをタブロイド風に誇張 | 毎日 JST 7:10 |
| 👤 Citizen1 | 会社員。感情的見出しに流されやすい | 毎日 JST 7:20 |
| 👴 Citizen2 | 高齢者。政府・健康・お金に反応しやすい | 毎日 JST 7:25 |
| 🔍 Factchecker | 要検証記事をWeb検索で検証 | 無効（disabled） |
| 📊 Observer | 全体集計・レポート生成・Telegram通知 | 毎日 JST 7:50 |
...
# 3. 設定ファイルを用意
cp .env.example .env               # APIキー・botトークンを記入
